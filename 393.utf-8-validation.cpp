...
public:
    bool validUtf8(vector<int>& data) {
        ...
        while(i<data.size()){
            if(data[i]<128){
                ...
                if(is_Utf8(data,i,i+1)==false) return false;
                i=i+2;
            }
            else if(data[i]>=224&&data[i]<240){
                if(is_Utf8(data,i,i+2)==false) return false;
                i=i+3;
            }
            else if(data[i]>=240&&data[i]<248){
                if(is_Utf8(data,i,i+3)==false) return false;
                i+=4;
            }
            else{
                return false;
    ...
    bool is_Utf8(vector<int>& data,int i,int j){
        //判断 data中[i,j]（闭区间）是否是合法的utf 8字符
